...
class Wastageinventory extends Backend_Controller
{
	...
	public function __construct()
	{
		...
		$this->exportUrl = base_url() . 'backend/reports/wastageinventory/export';
		$this->exportFileName = 'wastage-inventory-report';
		$this->exportRedirectUrl = base_url() . 'backend/reports/wastageinventory/';
	...
	{
		$this->navTitle = $this->pageTitle = 'Wastage Inventory Report';

		$data['dropdownWastageStocks'] = $this->wastagestock->getWastageStocksDropdown();
		$data['viewFile'] = 'backend/wastage-inventory-report/index';
	...
	public function fetchReport()
	{
		...
		$wastageStockNumber = intval($this->input->post('wastageStockNumber'));
		...
		try
		{
			...
			$response['data'] = $this->wastagestock->getWastageStockProducts($wastageStockNumber, $startDateTimestamp, $endDateTimestamp, $categoryIds);
			...
			foreach ($results['data'] as $key => &$result)
			{
				if ($key == 0)
				{
					$columns = [
						...
						['title' => 'Wastage Stock Number', 'name' => 'wastageStockNumber'],
